...
def get_clusters(treenames, filenames):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(filenames)) as executor:
        ...
        futures = {executor.submit(lambda thisfile: ROOT.TFile.Open(thisfile), filename): (treename, filename) for treename, filename in zip(treenames, filenames)}

        for future in concurrent.futures.as_completed(futures):
            treename, filename = futures[future]
            try:
                tree = future.result().Get(treename)
...
if __name__ == "__main__":
    ...
    with open("benchmarks/get_clusters_futures.csv", "a+") as f:
        for _ in range(5):
