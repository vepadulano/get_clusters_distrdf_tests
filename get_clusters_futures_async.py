...
def get_clusters(treenames, filenames):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(filenames)) as executor:
        ...
        futures = {executor.submit(asyncopen, filename): (treename, filename) for treename, filename in zip(treenames, filenames)}
        concurrent.futures.wait(futures)


    filesdata = [(ROOT.TFile.Open(future.result()), treename, filename) for future, (treename, filename) in futures.items()]

    for (file, treename, filename) in filesdata:
        try:
